...
class CommonsensePrompter(object):
    ...
    def __init__(self, tokenizer, kg, template_name: str = "", verbose: bool = False):
        ...
        #self.atomic_dict = get_atomic_dict()
            self.retrievers = WordnetRetriever("/root/autodl-tmp/data/kgs/")
        elif kg == 'wi':
            self.retrievers = Conceptnet_retriever()
            self.retrievers.init("/root/autodl-tmp/data/kgs/wikidata/wiki_entity_name.txt")
        elif kg == 'cn':
            self.retrievers = Conceptnet_retriever()
            self.retrievers.init("/root/autodl-tmp/data/kgs/conceptnet/concept.txt")
