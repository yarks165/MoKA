...
def main(
    ...
    output_file: str = None,  # 添加新参数用于指定输出文件
):
    if output_file is None:
        output_file = f"results_{dataset}.txt"
    ...
    #model = model.cuda()
    if is_KG:
        kg_embed_path = "/root/autodl-tmp/data/kgs/conceptnet/ent.npy"
    ...
    if dataset == 'csqa':
        file = open('/root/autodl-tmp/data/commonsenseqa/devc.json', 'r', encoding='utf-8')
    elif dataset == 'siqa':
        file = open('/root/autodl-tmp/data/SIQA/devc.json', 'r', encoding='utf-8')
    else:
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"测试数据集：{dataset}\n")
        f.write(f"是否使用知识图谱：{is_KG}\n")
        f.write(f"基础模型：{base_model}\n")
        f.write(f"Lora权重：{lora_weights}\n")
        f.write(f"提示模板：{prompt_template}\n\n")
        f.write(f"平均概率：{prob_answers}\n")
        f.write(f"准确率：{acc / num}\n")
        f.write(f"正确样本数：{acc}\n")
        f.write(f"NOP样本数：{nop}\n")
        f.write(f"总样本数：{num}\n\n")
        ...
        for i, detail in enumerate(sample_details):
            ...
            f.write(f"预测: {detail['prediction']}\n")
            f.write(f"正确答案: {detail['answer']}\n")
            f.write(f"是否正确: {'True' if detail['is_correct'] else 'False'}\n\n")
...
def load_KG_module(model, path, config):
    for idx, layer in enumerate(model):
        if idx + 1 in config.layer_insertion:
            ...
            layer.mlp.kg_module.load_state_dict(buffer, strict=False)
            moe_path = os.path.join(path, f"MoE_retrieve_{idx}.bin")
            moe_buffer = torch.load(moe_path, map_location=torch.device("cuda"))
            layer.mlp.moe.load_state_dict(buffer, strict=False)
