...
def init_weight_KG(model, embedding_path, config):
    ...
    for idx, layer in enumerate(model):
        if idx + 1 in list(config.layer_insertion):
            layer.mlp.kg_module.init(embedding_path)
            layer.mlp.moe.init_weight()
            # if hasattr(layer.mlp, 'experts'):
            #     for expert in layer.mlp.experts:
            #         expert.kg_module.init(embedding_path)
            # else:
            #     layer.mlp.kg_module.init(embedding_path)
    return model
...
def train(
    ...
    data_path: str = "/root/autodl-tmp/data/alpaca_data_cleaned.json",
    output_dir: str = "",
    embedding_path: str = "/root/autodl-tmp/data/kgs/conceptnet/ent.npy",
    ...
    batch_size: int =128,
    kg_path: str = "/root/autodl-tmp/data/kgs/conceptnet/concept.txt",
...
def save_KG_module(model, path, config):
    ...
    for idx, layer in enumerate(model):
        if idx + 1 in config.layer_insertion:
            ...
            torch.save(layer.mlp.kg_module.state_dict(), tmp)
            moe_path = os.path.join(path, f"MoE_retrieve_{idx}.bin")
            torch.save(layer.mlp.moe.state_dict(), moe_path)
